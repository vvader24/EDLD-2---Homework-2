...
library(colorspace)
#library(ggplot2) # loaded by tidyverse
library(here)
library(janitor)
library(lubridate)
library(rio)
...
# Note I made the figure height a bit larger so it fits better
```
...
# I would put all these in a call to `mutate()`
...
# Instead of the below, use `mutate()` with `replace_na()`
